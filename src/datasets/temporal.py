...
def make_temporal_split(
    ...
    if rollout_steps > 0:
        if step_size != 1:
            raise ValueError(
                "rollout holdout currently requires one-step temporal pairs"
            )
        ...
        rollout_frames = rollout_start + np.arange(rollout_steps + 1, dtype=np.int64)
...
def compute_global_normalization_params(
    ...
    if coord_count == 0 or flow_count == 0:
        raise ValueError(
            "Cannot compute normalization statistics from an empty training split"
        )
...
def copy_normalization_params(params: dict[str, np.ndarray] | None):
    if params is None:
        return None
    return {
        key: np.asarray(value, dtype=np.float32).copy() for key, value in params.items()
    }
