...
from .FNO_Layers import SpectralConv1d, SpectralConv2d, SpectralConv3d, SpectralConv2d_IrregularGeo, IPHI
    ...
    def __init__(
        ...
        if self.geotype == 'unstructured':
            self.fftproject_in = SpectralConv2d_IrregularGeo(
                n_hidden, n_hidden, modes, modes, shapelist[0], shapelist[1]
            )
            self.fftproject_out = SpectralConv2d_IrregularGeo(
                n_hidden, n_hidden, modes, modes, shapelist[0], shapelist[1]
            )
            self.iphi = IPHI()
            self.padding_list = [(16 - size % 16) % 16 for size in [shapelist[0], shapelist[1]]]
        else:
